  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-slate-600 mb-6">
        We will validate your answers and prepare your ESTA. We never log sensitive details in the
        browser console.
      </p>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        <Field label="Given name"><input className="input" {...register('givenName')} /></Field>
        <Error error={errors.givenName} />
        <Field label="Family name"><input className="input" {...register('familyName')} /></Field>
        <Error error={errors.familyName} />
        <Field label="Email"><input className="input" type="email" {...register('email')} /></Field>
        <Error error={errors.email} />
        <Field label="Nationality (country of passport)">
          <input className="input" {...register('nationality')} />
        </Field>
        <Error error={errors.nationality} />
        <Field label="Passport number"><input className="input" {...register('passportNumber')} /></Field>
        <Error error={errors.passportNumber} />
        <Field label="Date of birth (YYYY‑MM‑DD)">
          <input className="input" placeholder="1980-01-31" {...register('dob')} />
        </Field>
        <Error error={errors.dob} />
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <Field label="Planned arrival date (optional)">
              <input className="input" placeholder="2025-12-15" {...register('plannedArrival')} />
            </Field>
          </div>
          <div>
            <Field label="Planned departure date (optional)">
              <input className="input" placeholder="2026-01-02" {...register('plannedDeparture')} />
            </Field>
          </div>
        </div>
        <div className="card">
      ...
      <p className="text-xs text-slate-500 mt-6">
        We are not affiliated with the U.S. Government. You can apply directly with CBP and pay only
        the official fee.
      </p>
    </div>
  )
